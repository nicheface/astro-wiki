 ...
 * Two tools: Content Q&A + Article Generator
 */
"use client";
...
export default function Studio({ itemsJson, referenceDocsJson, apiKey }: StudioProps) {
  ...
  // Debug
  useEffect(() => {
    console.log("[Studio] Loaded:", articles.length, "articles,", referenceDocs.length, "docs");
  }, []);
  ...
  return (
    <div className="not-prose">
      <div className="mb-8">
        ...
        <p className="text-sm text-zinc-500 dark:text-slate-400 mt-1">AI 问答 · 文章生成</p>
      </div>
      ...
      <div className="flex gap-1 mb-8 p-1 rounded-xl bg-zinc-100 dark:bg-slate-800 w-fit">
        <button
          onClick={() => setTab("qa")}
          className={`px-4 py-1.5 rounded-lg text-sm font-medium transition-all ${tab === "qa" ? "bg-white dark:bg-slate-700 text-zinc-900 dark:text-slate-100 shadow-sm" : "text-zinc-500 dark:text-slate-400 hover:text-zinc-700 dark:hover:text-slate-300"}`}
        >
          💬 问答
        </button>
        <button
          onClick={() => setTab("generate")}
          className={`px-4 py-1.5 rounded-lg text-sm font-medium transition-all ${tab === "generate" ? "bg-white dark:bg-slate-700 text-zinc-900 dark:text-slate-100 shadow-sm" : "text-zinc-500 dark:text-slate-400 hover:text-zinc-700 dark:hover:text-slate-300"}`}
        >
          ✨ 生成文章
        </button>
      </div>
