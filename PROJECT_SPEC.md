...
1. **能否复用已有组件？** 审视 `src/components/` 下全部组件（DataChart / HoverDetail / SmartImageCard / LiveCodeBlock / SwotCard / StepFlow / ArticleTag），问自己哪些内容可以用组件表达而不是文字。

2. **能否创建全新组件？** 如果已有组件不够用，主动创建。一个组件的一次性成本换来的是全站永久的表达力增益。

3. **读者体验是否生动直观？** 每写完一章，问自己：读者看到的是可视化还是干巴巴文字？对比是否并排？流程是否一目了然？术语是否有悬浮解释？

**目标**：让读者离开时不仅多了一份理解，还觉得"这个知识库真有意思"。
...
> 📖 **AI 须知**：在开始编写任何代码前，阅读本文件（至少第 2-9 节）。编写笔记前，额外阅读 `NOTE_WRITING_GUIDE.md`。如果遇到新的技术决策或踩坑经验，请更新本文件并将变更提交到 Git。
