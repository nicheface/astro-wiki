...
**创建新组件时的铁律**（来自 SwotCard 三次迭代的教训）：

1. **不要用通用组件凑合。** 不能用 DataChart 去模拟 SWOT 卡片，不能用 SmartImageCard 去冒充流程图。该建就建。
2. **先研究参考设计。** 如果有对标参照（如原文章的 SwotCard），先仔细拆解：布局是并排还是堆叠？配色是多少？字体层级是什么？不要扫一眼就开始写。
3. **API 优先考虑调用方。** 组件是给别人（和未来的自己）用的。属性名要直观，结构要扁平。不要把两个语义不同的数据揉进一个数组。
4. **标题、分隔线、字重层级是信息设计的一部分。** 不是"有就行"，而是"读者扫一眼就能区分主次"。
